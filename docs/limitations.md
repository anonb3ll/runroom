...
Runroom is pre-release. The packaged CLI is not on PyPI, interfaces will change
without deprecation cycles, and there is no upgrade path between pre-1.0
versions. Treat anything you build against it as provisional.
...
- Audit history is append-only within the process; nothing yet prevents an
  operator with filesystem access from deleting the file. Ledger signing is a
